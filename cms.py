...
import time
import json
#import email_pattern_detection as E
import requests
import validators
...
for line in files:
    ...
    prefix = ["https://www."+str(line)]
    for y in prefix:
        print ("--------------------------> "+y)
        hb = is_hubspot(y)
        if hb:
            print "Hubspot detected"    
            write_in_a_file(line+";Hubspot;;",outputfile)
            #pass        
        ...
        if wordpress:
            ...
            print cms_version
            write_in_a_file(line+";WordPress;"+str(cms_version)+";"+str(cms_plugin),outputfile)
            #pass
 
        shopify = is_shopify(y)
        if shopify:
            print y+" Shopify detected"
            write_in_a_file(line+";Shopify;;",outputfile)
            #pass
    ...
    #drupal = is_drupal(line)
    #if drupal:
    #    break
    
    #pshop = is_prestashop(line)
    #if pshop:
    #    break
